...
def test_load_canon_row_count(lang, domain, expected_count):
    """Each (lang, domain) YAML loads exactly the expected number of terms."""
    terms = load_canon(lang, domain)
...
def test_term_surface_is_string():
...
def test_term_lemmas_length_matches_whitespace_tokens():
...
def test_en_color_terms_include_red():
...
def test_hyphenated_term_lemmas_one_per_whitespace_token():
